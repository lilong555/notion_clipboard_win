...
- 内置配置页面，不需要手写配置文件，并可直接录制新的全局热键、验证配置和测试上传。
- 上传失败会自动进入队列，之后可以继续重试。
- 托盘菜单可以打开上传中心，查看最近记录、队列状态、重试失败任务、最新 Obsidian 笔记和配置诊断。
...
1. 点击配置页面里的“验证配置”，检查 Notion token、data source、Obsidian vault 和文件夹是否可用。
2. 没有错误后，点击“测试上传”。程序会用当前页面里的配置真实写入一条测试内容。
3. 查看自动打开的“上传中心”，确认 Notion 页面链接或 Obsidian 文件路径是否正确。
4. 确认无误后，再点击“应用并重启”让配置成为正式配置。
...
可以先打开配置页面，点击“验证配置”查看诊断；诊断通过后再点“测试上传”确认是否能真实写入。
...
程序支持常见的 `$...$`、`$$...$$`、`\(...\)`、`\[...\]`。如果原文格式很松散，先用托盘菜单里的“预览 Obsidian Markdown”检查本地转换结果；开发调试时也可以把问题样例放到 `test/` 后运行 dry run。
