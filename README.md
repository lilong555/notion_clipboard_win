...
- 默认热键 `Ctrl+Shift+B` 保存当前剪贴板。
- 支持同时写入 Notion 和 Obsidian。
- 支持 Markdown、HTML、代码块、表格、行内公式和整行公式。
- 内置配置页面，不需要手写配置文件，并可直接录制新的全局热键和测试保存。
- 保存失败会自动进入队列，之后可以继续重试。
- 托盘菜单可以保存当前剪贴板、打开保存记录、查看日志和打开最新 Obsidian 笔记。
...
2. 下载最新的 `NotionClipboardWin-版本号-Setup.exe` 安装包。
3. 双击安装。
...
如果你刚新建了仓库或文件夹，但下拉列表还没显示，点击配置页面里的“重新扫描 Obsidian”。它会按当前页面里尚未保存的仓库路径重新扫描，不会写入 ini 配置。
...
1. 点击配置页面里的“测试保存”。程序会用当前页面里的配置真实写入一条测试内容。
2. 查看自动打开的“保存记录”，确认 Notion 页面链接或 Obsidian 文件路径是否正确。
3. 如果测试失败，按保存记录里的错误提示修正配置后再试一次。
4. 确认无误后，点击“应用并重启”让配置成为正式配置。

“测试保存”会写入一条名为 `Notion Clipboard Win 测试保存` 的内容。它不会把配置保存到 ini；只有点击“应用并重启”才会保存配置。

### 7. 调整热键

默认保存热键是 `Ctrl+Shift+B`。如果这个组合键和其他软件冲突，可以在配置页面修改：
...
2. 按你配置的保存热键，默认是 `Ctrl+Shift+B`。
3. 等待托盘通知，或在托盘菜单中打开“保存记录”。

你也可以右键托盘图标，选择“保存当前剪贴板”。

### 9. 查看保存结果

保存成功后：

- Notion：保存记录会显示 Notion 页面链接。
- Obsidian：文件会出现在你配置的仓库和子目录中。
- 托盘菜单里的“打开最近 Obsidian 笔记”可以直接打开最新保存的笔记。

如果保存失败，任务会保存在本地队列里，之后会继续重试。保存记录会显示等待重试和最终失败的任务；对最终失败的任务，可以点击“重试此项”单独重试，也可以点击“重试失败任务”批量放回队列。

保存记录是本地快照页面。如果页面已经打开了一段时间，点击“刷新状态”会重新生成最新状态。
...
如果你刚在磁盘中新建了文件夹，回到配置页面点击“重新扫描 Obsidian”。重新扫描只刷新列表，不会保存配置；确认无误后仍需要点击“应用并重启”。
...
可以先打开配置页面，点击“测试保存”确认是否能真实写入；失败原因会显示在保存记录。
...
如果你不使用配置页面，也可以编辑 `notion_clipboard_win.ini`。普通用户仍建议优先使用配置页面，因为热键录制、Obsidian 仓库选择和测试保存都在页面里更直观。
...
CTest 会同时运行内置自测，以及 `test/bf.txt` 的 Notion 与 Obsidian 长文本转换回归。CI 还会执行发版门禁快检和安装包脚本回归；提交前建议至少跑上面的 CTest。
