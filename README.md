...
本地 `--self-test` 覆盖普通算法讲解、HTML 空代码块污染、HTML 列表项段落、`text`/未知代码语言降级、Markdown/LaTeX、任务列表、引用块、表格保守保留、多行内公式拆分、超长代码块拆分、超长公式降级、货币 `$` 误判、inline code 中的 `$`、URL/path 中的 `$`、多反引号 code span、Windows 路径、HTML 数字实体、KaTeX/MathJax、script/style 污染和 append 请求体切分。
