...
document.getElementById("testUpload").addEventListener("click",()=>{build(); const problems=validateConfig(); if(problems.length){updateStatus(); return;} statusBox.className="status ok"; statusBox.textContent="已发送测试上传请求，结果会写入并打开最近上传结果。"; location.href=protocolUrlWithOutput("test-upload");});
