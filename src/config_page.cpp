...
<main class="wrap"><div>
    ...
    html << R"(
</div><aside class="output"><h2>保存配置</h2><p class="hint">完成后点击“应用并重启”，托盘进程会读取新设置。需要先试写一条内容时，点击“测试保存”。</p><div id="status" class="status" role="status"></div><div class="actions"><button id="apply">应用并重启</button><button id="testUpload" class="secondary">测试保存</button><button id="openUploadCenter" class="secondary">保存记录</button><button id="refreshObs">重新扫描 Obsidian</button><button id="reveal" class="secondary">显示/隐藏 token</button></div><details class="advanced"><summary>高级：查看或导出 ini</summary><p>这里包含 token，仅用于手动备份或迁移配置。</p><div class="path">配置文件：)"
...
const testUploadButton=document.getElementById("testUpload");
...
function obsidianFolderProblem(folder){folder=(folder||"").trim(); if(!folder)return ""; if(folder.startsWith("/")||folder.startsWith("\\"))return "Obsidian 子目录必须是仓库内相对目录，不能以 / 或 \\ 开头"; const parts=folder.replace(/\\/g,"/").split("/").map(part=>part.trim()).filter(Boolean); for(const part of parts){if(part===".")continue; if(part==="..")return "Obsidian 子目录不能包含 .."; if(part.includes(":"))return "Obsidian 子目录不能包含盘符或冒号";} return "";}
function validateConfig(){const selected=selectedTargets(); const problems=[]; if(!selected.length)problems.push("至少选择一个保存位置"); if(!isHotkeyTextValid(val("hotkey")))problems.push("全局热键格式无效，请重新录制类似 Ctrl+Shift+B 的组合键"); if(selected.includes("notion")){if(!val("notion_token"))problems.push("Notion Token 不能为空"); if(!val("data_source_id")&&!val("database_id"))problems.push("Notion 需要数据源 ID 或 Database ID");} if(selected.includes("obsidian")){if(!val("obsidian_vault_dir"))problems.push("Obsidian 仓库不能为空"); const folderProblem=obsidianFolderProblem(val("obsidian_folder")); if(folderProblem)problems.push(folderProblem);} return problems;}
function updateStatus(){const selected=selectedTargets(); const problems=validateConfig(); statusBox.className="status "+(problems.length?"error":"ok"); statusBox.textContent=problems.length?("需要处理："+problems.join("；")):("配置完整。保存后会写入："+selected.map(targetLabel).join("、")); const disabled=problems.length>0; applyButton.disabled=disabled; testUploadButton.disabled=disabled;}
...
testUploadButton.addEventListener("click",()=>{build(); const problems=validateConfig(); if(problems.length){updateStatus(); return;} statusBox.className="status ok"; statusBox.textContent="已发送测试保存请求，结果会写入并打开保存记录。"; location.href=protocolUrlWithOutput("test-upload");});
            ...
            for (const char *needle : {"notion_secret", "value=\"obsidian\" data-target-option=\"obsidian\"",
                                       ...
                                       "normalizePathKey(path)", "findObsidianFolderGroup(vault)",
                                        ...
                                        "selected.map(targetLabel).join(\"、\")", "syncTargets()",
                                         ...
                                         "已发送测试保存请求，结果会写入并打开保存记录。",
                                         "id=\"openUploadCenter\"", "保存记录",
                                         "protocolUrl(\"open-upload-center\")",
                                       "id=\"status\"", "validateConfig()", "applyButton.disabled",
                                       "testUploadButton.disabled", "const testUploadButton=",
                                       "配置完整。保存后会写入：", "需要处理：",
                                       "obsidianFolderProblem(folder)", "Obsidian 子目录不能包含 ..",
                                       "Obsidian 子目录不能包含盘符或冒号",
